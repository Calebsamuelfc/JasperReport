...
public class SvipServiceImpl implements SvipService {
    @Override
    public ServiceResponse downloadSvipCsv(ContractRequestDTO contractRequestDTO) {
        ...
        try {
            ...
            try (CSVWriter csvWriter = new CSVWriter(csvWriterString)) {
                String[] headers = getHeaders(rootNode.get("svipData").get(0));
                csvWriter.writeNext(headers);
                for (JsonNode node : rootNode.get("svipData")){
    ...
    @Override
    public ServiceResponse downloadSvipExcel(ContractRequestDTO contractRequestDTO) {
        byte[] decodedBytes = Base64.getDecoder().decode(contractRequestDTO.getBase64String());
        String decodedString = new String(decodedBytes);
        String base64String;
        try {
